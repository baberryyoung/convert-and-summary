...
parser.add_argument("-o","--operation", help="the operation type of this dataset used for(train/dev/test)", default="出院小结hjh-annotation")
...
def merge_ann(data_file, operation):
    ...
    for i in range(len(entities)):
        ...
        if entities[i][0] in ['test','test_result','body_location','operation','drug','negation','disorder','negative_symptom','image_feature','pathology_feature','size','number','exporesure','time_point','period','person','uncertainty']:
            if entities[i][0] in ['period','time_point']:
                label='time'
            elif ('强化' in entities[i][3] or '影' in entities[i][3] or '肿' in entities[i][3] or '结节' in entities[i][3]) and entities[i][0] != 'image_feature':
                label = 'image_feature'
            else:
                label = entities[i][0]
            if word_len > 1:
                annote.extend([label+'-B'])
                annote.extend((word_len-2) * ([label+'-I']))
                annote.extend([label + '-E'])
            else:
                annote.extend([label + '-S'])
        else:
...
'''
    for _, _, files in os.walk(raw_data_file):
        ...
        for file in files:
            ...
            _, sentence_num = merge_ann(os.path.join(raw_data_file, file.split('.')[0]),
                                        args.operation)  # train,dev,test
